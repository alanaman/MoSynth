...
namespace MotionMatching
{
    public class CrowdControlInput : MotionMatchingControlInput, IObstacleAwareCharacterControler, IPlayerInputCharacterController
    {
        ...
        protected override void OnUpdate()
        {
            ...
            if (DoSteering)
            {
                var targetSteering = ComputeSteering(currentPos, transform.forward, Obstacles, SteeringLookAhead, SteeringForce, debug: DebugSteering);
        ...
        private void ClampMotionMatching()
        {
            ...
            {
                var newMotionMatchingPos = MaxDistanceMMAndCharacterController * math.normalize(mmPos - characterController) + characterController;
        ...
        private void AdjustCharacterPosition()
        {
            ...
            var adjustmentPosition = Spring.DampAdjustmentImplicit(differencePosition, PositionAdjustmentHalflife, Time.deltaTime);
        ...
        private void AdjustCharacterRotation()
        {
            ...
            // Damp the difference using the adjustment halflife and dt
            var adjustmentRotation = Spring.DampAdjustmentImplicit(differenceRotation, RotationAdjustmentHalflife, Time.deltaTime);
            // Clamp adjustment if the length is greater than the character angular velocity
            // multiplied by the ratio
            var maxLength = RotMaximumAdjustmentRatio * math.length(base.motionSynthesizer.RootAngularVelocity) * Time.deltaTime;
                                             ...
                                             bool debug = false)
        {
            ...
            for (var i = 0; i < numRays; i++)
            {
                ...
                for (var j = 0; j < obstacles.Length; j++)
                {
                    ...
                    if (obstacles[j].Intersect(currentPos, rayDirection, out var hitPoint1, out var hitDistance1, out var hitPoint2, out var hitDistance2))
                    {
                ...
                if (resHitDistance < lookAhead)
                {
                    ...
                    var localSteering = math.normalize(forwardProj) * force * math.max(0.0f, math.log10(1.0f - (resHitDistance / lookAhead)) + 1.0f);
