...
public struct PoseVector
{
    ...
    /// after applying FK.
    /// </summary>
    public quaternion GetRootSpaceRotation(Skeleton skeleton, Skeleton.Joint joint)
    {
    ...
    /// after applying FK.
    /// </summary>
    public float3 GetRootSpacePosition(Skeleton skeleton, Skeleton.Joint joint)
    {
        var localToCharacter = float4x4.identity;
        while (joint.index != 0) // while not root
        {
            var localRot = math.float3x3(JointLocalRotations[joint.index]);
            var localTransform = math.float4x4(localRot, JointLocalPositions[joint.index]);
    ...
    /// after applying FK.
    /// </summary>
    public float3 GetRootSpaceVelocity(Skeleton skeleton, Skeleton.Joint joint)
    {
        var posAcc = float3.zero;
        var linVelAcc = float3.zero;
        var angVelAcc = float3.zero;
    ...
    /// after applying FK.
    /// </summary>
    public float3 GetRootSpaceAngularVelocity(Skeleton skeleton, Skeleton.Joint joint)
    {
        var angVelAcc = float3.zero;
